        ...
        <div className="pb-24 pt-6 sm:pt-8">
            {searchTerm && (
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    ...
                    <p className="text-gray-500 mt-2 font-medium">{filteredServices.length} tool{filteredServices.length !== 1 ? 's' : ''} found</p>
                </div>
            )}
            ...
            <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {filteredServices.map((service) => {
                        ...
                        return (
                            <article
                                ...
                                className={`service-card opacity-0 bg-white/70 backdrop-blur-xl rounded-[2rem] p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-2xl transition-colors duration-300 border border-gray-100 flex flex-col items-start group ${colors.shadow}`}
                            >
                                ...
                                <Link
                                    to={`/${service.id}`}
                                    className={`w-full flex items-center justify-center gap-2 px-6 py-3.5 ${colors.btn} text-white font-bold rounded-xl transition-all duration-300 text-sm shadow-lg group-hover:shadow-xl focus:ring-2 focus:ring-offset-2`}
                                >
                                    Use Tool
                ...
                {filteredServices.length === 0 && (
                    <div className="text-center py-24 bg-white/50 backdrop-blur-sm rounded-[2.5rem] border border-gray-200 shadow-sm mt-8 max-w-3xl mx-auto">
