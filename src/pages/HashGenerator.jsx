...
const HashGenerator = () => {
  ...
  return (
    <div className="h-full min-h-0 flex flex-col max-w-6xl mx-auto w-full animate-in fade-in duration-500">
      <div className="mb-4">
        <div className="bg-amber-50 text-amber-600 w-14 h-14 rounded-2xl flex items-center justify-center mb-3 border border-amber-100"><Fingerprint size={28} /></div>
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight mb-2">Hash Generator</h1>
        <p className="text-base text-gray-600 font-medium">Generate browser-supported cryptographic hashes locally with the Web Crypto API.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-4 flex-1 min-h-0">
        <section className="bg-white/70 rounded-[2rem] border border-gray-100 shadow-sm p-4 flex flex-col min-h-0">
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)} className="w-full flex-1 min-h-0 p-5 rounded-2xl border border-gray-200 bg-gray-50 font-mono text-sm focus:ring-4 focus:ring-amber-100 focus:border-amber-400 outline-none resize-y" />
        ...
        <aside className="bg-white/70 rounded-[2rem] border border-gray-100 shadow-sm p-4 flex flex-col min-h-0">
      ...
      <section className="mt-4 bg-white/70 rounded-[2rem] border border-gray-100 shadow-sm p-4 flex flex-col min-h-0">
        ...
        <textarea readOnly value={hash} className="w-full h-24 p-5 rounded-2xl border border-gray-200 bg-gray-50 font-mono text-sm outline-none resize-y" placeholder="Your hash appears here..." />
      </section>
    </div>
  );
};
